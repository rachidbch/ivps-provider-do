...
DigitalOcean provider plugin for [ivps](https://github.com/rachidbch/ivps). Single-file Bash library (`plugin.sh`) implementing the IVPS plugin interface (cmd_keys/cmd_validate/cmd_create/cmd_delete/cmd_list/cmd_show). Bundles `cloud-init.yaml` for node bootstrapping.
...
Required functions (defined in `plugin.sh`, sourced by ivps inside a subshell):

```
cmd_keys                       # Output DO_API_TOKEN:DigitalOcean API Token
cmd_validate                   # Check DO_API_TOKEN via doctl; exit 0=ok, 2=bad/missing
cmd_create <name> [options]    # Provision a droplet (doctl --wait, no IP polling)
cmd_delete <name>              # Destroy a droplet (exit 3 if not found)
cmd_list                       # List droplets (doctl -o json array)
cmd_show <name>                # Show droplet details (doctl -o json single object)
```

Env vars consumed: `DO_API_TOKEN` (required), `IVPS_CONFIG_DIR`, `IVPS_PROVIDER_DIR`.

### `cmd_create` arguments

```
cmd_create <name> [--plan <slug>] [--region <slug>] [--image <slug>] [--cloud-init <path>]
```
...
- **Cleanup on failure** — If post-create steps fail (SSH wait, cloud-init wait, etc.), ivps calls `cmd_delete <name>`
...
test/
  ...
  03_dispatcher.bats    # Interface validation (type -t checks), cloud-init.yaml validation
...
5. `source_plugin_functions` loads all functions by sourcing `plugin.sh`
